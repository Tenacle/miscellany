...
int invShiftRows();
int invSubBytes();
int rotWord();          // probs not required
int shiftRows();
int subBytes();
int subWord();
...
int mixColumns(uint8_t **s, size_t mSize)
{
    ...
    {
        col[0] = (0x02*s[0][c])^(0x03*s[1][c])^s[2][c]^s[3][c];
        col[1] = s[0][c]^(0x02*s[1][c])^(0x03*s[2][c])^s[3][c];
        col[2] = s[0][c]^s[1][c]^(0x02*s[2][c])^(0x03*s[3][c]);
        col[3] = (0x03*s[0][c])^s[1][c]^s[2][c]^(0x02*s[3][c]);
