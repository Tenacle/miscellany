...
FrameList *createFrameList(int delta, int frNum, char *frame)
{
    ...
    frameList->frame = malloc(strlen(frame));
    strcpy(frameList->frame, frame);
    return frameList;
}
...
void *fL2R()
{
    ...
    while(fgets(line, MAX_INFO_SIZE, fptr) != NULL)
    {
        if(head == NULL)
        {
            ...
            head = createFrameList(-1, index, newFrame);
            printf("created by stuffed frame:\n");
            printf("%s", newFrame);
        }
        index++;
    }
    ...
    while(currFrame != NULL)
    {
        ...
        pthread_mutex_lock(&physical->L2RLock);
        while(!&physical->L2RRxSignal)
        {
            pthread_cond_wait(&physical->L2RRxSignal, &physical->L2RLock);
        }
        // transmit
        
        transmitFrame(L2R, (unsigned char *)currFrame->frame, sizeof(currFrame->frame));
        pthread_mutex_unlock(&physical->L2RLock);
    }
...
int main()
{
    ...
    char *frame = createFrame("hello world~ I am a frame~ woopediedooda~", CTL_SS_INF, CTL_F, 0, 0);
    printf("PRINTING EOF: %c\n", EOF);
    printf("PRINTING EOF: %d\n", EOF);
    printf("Testing frame with size %lu: %s\n", strlen(frame+4), frame+4);
    printf("Printing the whole frame\n");
    for(int i = 0; i < 100; i++)
    {
        printf("%c", frame[i]);
    }
    ...
    pthread_create(&tL2R, NULL, &fL2R, NULL);
    pthread_create(&tR2L, NULL, &fR2L, NULL);
...
{
    char stuffedInfo[MAX_INFO_SIZE];
    int padding = 0;
...
char *createFrame(char *info, uint8_t ss, uint8_t pf, int rFrNum, int sFrNum)
{
    ...
    index += strlen(stuffedInfo)+1;                     // don't need \0. Will be replaced
